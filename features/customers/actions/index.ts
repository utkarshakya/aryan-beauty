...
export async function updateMyProfile(
  ...
  const customer = await prisma.customer.findUnique({ where: { userId: appUser.id } });
  if (!customer) return { errors: { form: "Customer profile not found" } };
...
export async function linkWalkInCustomerAction(
  ...
  const appUser = await prisma.user.findUnique({ where: { clerkUserId } });
  if (!appUser) {
